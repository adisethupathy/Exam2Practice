...
class Box(object):
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > self.volume:
            ...
            for i in range(count, len(self.contents)):
                additional_string = additional_string + self.contents[i]
    ...
    def double_then_shrink(self, new_volume):
        ...
        sub_string1 = self.double()
        sub_string2 = self.shrink(new_volume)

        return len(sub_string1)+ len(sub_string2)
    ...
    def reset(self):
        ...
        self.history_list += [self.contents]
    ...
    def steal(self, other_box):
        ...
        string = self.append_string(other_box.contents)
        other_box.contents = string
